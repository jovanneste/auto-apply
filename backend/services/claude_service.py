...
async def parse_cv_to_profile(raw_cv_text: str) -> dict[str, Any]:
    response = await _client.messages.create(
        ...
        messages=[{"role": "user", "content": f"Extract structured data from this CV:\n\n{raw_cv_text}"}],
    )
...
async def map_fields_to_profile(
    ...
    # Ensure we always return one entry per field
    while len(result) < len(fields):
        result.append({"proposed_answer": None, "confidence": "missing", "reasoning": "Not returned by Claude", "source": "missing"})
...
async def generate_instructions_stream(job) -> AsyncGenerator[str, None]:
    ...
    for page_num in sorted(fields_by_page):
        ...
        for f in fields_by_page[page_num]:
            ...
            flag = " ⚠️ NEEDS YOUR INPUT" if not (f.final_answer or f.proposed_answer) else ""
